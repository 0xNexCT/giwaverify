...
contract GiwaFaucetV3 {
    ...
    function claimAll() external {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (block.timestamp < lastClaimTime[msg.sender][token] + COOLDOWN) continue;
            if (IERC20(token).balanceOf(address(this)) < CLAIM_AMOUNT) continue;
            if (totalClaimedPerWallet[msg.sender][token] + CLAIM_AMOUNT > MAX_PER_WALLET) continue;
